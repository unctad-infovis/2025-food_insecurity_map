...
function ChartMap({ values }) {
  ...
  const createMap = useCallback((data, topology) => {
    ...
    const pointFormat = () => 'Index: <strong>{point.value:.1f}</strong>';
    ...
    chartMapRef.current = Highcharts.mapChart('map_container', {
      caption: {
        ...
        text: '<em>Source:</em> UN Trade and Development (UNCTAD) based on FAO Note: Countries with a negative import dependency ratio are net exporters of cereals. Cereal import dependency does not imply that countries are overall net food importers or have a net negative agricultural balance.<br /><em>Note:</em> Data in light grey are not available, <a href="https://unctad.org/page/map-disclaimer" target="_blank">Map disclaimer</a>, https://unctad.org/map-disclaimer',
        useHTML: true,
      ...
      exporting: {
        ...
        enabled: false,
        filename: '2025-food_insecurity_map'
      },
      legend: {
        align: 'left',
        enabled: false,
